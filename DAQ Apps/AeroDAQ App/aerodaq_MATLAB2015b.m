...
%   	DIO 1 PIN25  - Motor controller steps or Scannivalve next
...
h.do.checkbox.motor  = uicontrol(               ...
    ...
    'Callback',         @motorcheckbox_callback);
h.do.text.nsteps  = uicontrol(                  ...
    h.do.panel,                                 ...
    'BackgroundColor',  [1.0 1.0 1.0],          ...
    'Style',            'text',                 ...
    'String',           'No. Steps',            ...
...
h.do.edit.nsteps  = uicontrol(                  ...
    ...
    'Callback',         @nsteps_callback);
h.do.text.direction  = uicontrol(               ...
    h.do.panel,                                 ...
    'BackgroundColor',  [1.0 1.0 1.0],          ...
    'Style',            'text',                 ...
    'String',           'Direction',            ...
    ...
    'HorizontalAlignment','left',               ...
    'Position',         [10 100 100 20]);
h.do.radiobutton.group = uibuttongroup(         ...
    'Parent',           h.do.panel,             ...
    'Title',            '',                     ...
    'BackgroundColor',  [1.0 1.0 1.0],          ...
    'BorderType',       'none',                 ...
    'Position',[.1 0 .8 .6]);
h.do.radiobutton.fdir  = uicontrol(             ...
    h.do.radiobutton.group,                     ...
    'BackgroundColor',  [1.0 1.0 1.0],          ...
    'Style',            'radiobutton',          ...
    'String',           'Pos.',                 ...
    'FontName',         h.fontname,             ...
...
h.do.radiobutton.bdir  = uicontrol(             ...
    h.do.radiobutton.group,                     ...
    'BackgroundColor',  [1.0 1.0 1.0],          ...
    'Style',            'radiobutton',          ...
    'String',           'Neg.',                 ...
...
h.do.text.position_label  = uicontrol(          ...
    h.do.panel,                                 ...
    'BackgroundColor',  [1.0 1.0 1.0],          ...
    'Style',            'text',                 ...
...
h.do.pushbutton.return  = uicontrol(            ...
    h.do.panel,                                 ...
    'BackgroundColor',  [1.0 1.0 1.0],          ...
    'Style',            'pushbutton',           ...
    'String',           'Go to Zero',           ...
...
h.do.pushbutton.zero  = uicontrol(              ...
    h.do.panel,                                 ...
    'BackgroundColor',  [1.0 1.0 1.0],          ...
    'Style',            'pushbutton',           ...
    'String',           'Set Zero',             ...
...
h.status.text  = uicontrol(                     ...
    ...
    'Position',         [312 585 440 20]);

%create axes
h.axes.ts    = axes('Position',[.42 .43 .3 .25]);
h.axes.mean  = axes('Position',[.42 0.08 .3 .25]);
h  = update_plot1(h);
h  = update_plot2(h);
...
nsteps     = str2double(get(h.do.edit.nsteps,'String'));
mdir       = get(h.do.radiobutton.fdir,'Value');
...
flag_do    = get(h.do.checkbox.motor,'Value');
flag_stop  = 0;
path_ts    = h.acq.timeseries.path;
flag_yestoall  = 0;
if get(h.do.radiobutton.fdir,'Value')
    direction  = 1;
else
    direction  = 0;
end
    ...
    for ai  = 1:N
        ...
        if flag_do
            ...
            if (isequal(ai,N) && flag_ai)
                ...
                %status update
                sdisp(h,sprintf('Moving %i steps in the %i direction.',nsteps,direction));

                %move nsteps
                do_move(nsteps,direction);
               % move_stepper(nsteps,direction);

                %change position
                h.position  = h.position + nsteps*(2*mdir-1);
                set(gcf,'UserData',h);
                ...
                set(h.do.text.position,'String',sprintf('%i',h.position));

                %status update
                sdisp(h,['Current position at ' num2str(h.position) '.']);
...
%--------------------------------------------------------------------
function motorcheckbox_callback(hObject,eventdata)

%get userdata
h  = get(gcf,'UserData');

%set the save checkboxes and pushbuttons appropriately
if get(h.do.checkbox.motor,'Value')
    set(h.do.edit.nsteps,'Enable','on');
    set(h.do.radiobutton.fdir,'Enable','on');
    set(h.do.radiobutton.bdir,'Enable','on');
    if h.position
        %set(h.do.pushbutton.return,'Enable','on');
        %set(h.do.pushbutton.zero,'Enable','on');
    end
    sdisp(h,'Digital out is enabled.');
else
    set(h.do.edit.nsteps,'Enable','off');
    set(h.do.radiobutton.fdir,'Enable','off');
    set(h.do.radiobutton.bdir,'Enable','off');
    %set(h.do.pushbutton.return,'Enable','off');
    %set(h.do.pushbutton.zero,'Enable','off');
    sdisp(h,'Digital out is disabled.');
end
...
%--------------------------------------------------------------------
function zero_callback(hObject,eventdata)

%get userdata
h  = get(gcf,'UserData');

%prompt user to make sure
choice  = questdlg('Are you sure you want to set the zero position?','Warning','Yes','No','No');
...
if isequal(choice,'Yes');
    ...
    %status update
    sdisp(h,'Position reset to 0.');
end


%--------------------------------------------------------------------
function return_callback(hObject,eventdata)

%get userdata
h  = get(gcf,'UserData');

%prompt user to make sure
choice  = questdlg('Are you sure you want to return the stepper motor to zero position?  This will also send the Scanivalve home.','Warning','Yes','No','No');
...
if isequal(choice,'Yes');
    ...
    %update status
    sdisp(h,sprintf('Moving %i steps in the %i direction.',abs(h.position),direction));
    
    %move motor to zero point
    do_move(abs(h.position),direction);
...
%--------------------------------------------------------------------
function nsteps_callback(hObject,eventdata)
...
function do_move(nsteps,direction)
...
putvalue(do_direction, direction);
...
%--------------------------------------------------------------------
function do_gohome  %return the scannivalve
