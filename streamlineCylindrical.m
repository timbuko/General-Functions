...
%%%%%%%%%%%%%%%%%
% edited from Matlab Streamline code. Enter cylindrical coordinates and
% velocities, then this will convert the output into xyz so it can plot as
% a 3D surface (edits are made to plotting part lines 86 and 89).  Enter
% radius of sphere as option 3 (you need to give options 1 and 2). NOTE YOU
% NEED TO INPUT VELOCITIES OF SAME UNITS, SO CONVERT DTHETA TO DS
%%%%%%%%%%%%%%%%%
...
for k = 1:length(verts)
  ...
  if ~isempty(vv)
      %%%%%% My Addition
      h = [h ; line('xdata', r*cos(vv(:,1)/r), 'ydata', r*sin(vv(:,1)/r), 'zdata', vv(:,2), ...
